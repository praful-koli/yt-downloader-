...
app.use((req, res, next) => {
  ...
  res.setHeader(
    ...
    "Content-Type, Accept, X-Cookie-Data",
  );
...
app.post("/download", (req, res) => {
  ...
  proc.on("close", (code) => {
    ...
    console.log(
      `[job ${id}] ${code === 0 ? "✅" : "❌"} ${job.filename || "failed"}`,
    );
    setTimeout(
      () => {
        try {
...
app.get("/file/:id/:filename", (req, res) => {
  ...
  console.log(
    `[stream] → ${name} (${(stat.size / 1024 / 1024).toFixed(1)} MB)`,
  );
...
app.get("/open-folder", (_req, res) => {
  ...
  try {
    ...
    if (process.platform === "win32")
      spawn("explorer", [p], { detached: true, shell: true });
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `║   ${IS_CLOUD ? "☁️  Render Cloud" : "💻 Local PC    "}                  ║`,
  );
  console.log("╚══════════════════════════════════════╝");
  console.log(`\n📡  Port   : ${PORT}`);
  console.log(`🔧  yt-dlp : ${getYtDlp() || "❌ NOT FOUND"}`);
  console.log(
    `🍪  cookies: ${fs.existsSync(COOKIE_FILE) ? "✅ found" : "⚠️  not uploaded yet"}`,
  );
  console.log("\n✅  Ready!\n");
});
