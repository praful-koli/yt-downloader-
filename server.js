...
app.post("/api/download", (req, res) => {
  ...
  if (!ytdlp)
    return res
      .status(500)
      .json({ error: "yt-dlp not installed on server. Contact admin." });
  ...
  // Use iOS client — works with cookies, no JS challenge needed
  parts.push('--extractor-args "youtube:player_client=ios"');

  // Also try node runtime as fallback
  try {
    const node = execSync("which node 2>/dev/null || where node 2>nul", {
      encoding: "utf8",
    })
      .split("\n")[0]
      .trim();
    if (node) {
      parts.push(`--js-runtimes "node:${node}"`);
  ...
  proc.on("close", (code) => {
    ...
    console.log(
      `[job ${id}] ${code === 0 ? "✅ done" : "❌ error"}: ${job.filename || "failed"}`,
    );
  ...
  res.json({
    ...
    fileUrl:
      job.status === "done" && job.filename
        ? `/api/file/${job.id}/${encodeURIComponent(job.filename)}`
        : null,
  });
});
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `🔧 yt-dlp  : ${getYtDlp() || "❌ not found — run: pip install yt-dlp"}`,
  );
  console.log(
    `🍪 cookies : ${fs.existsSync(COOKIE_FILE) ? "✅ loaded" : "⚠️  not uploaded yet"}`,
  );
